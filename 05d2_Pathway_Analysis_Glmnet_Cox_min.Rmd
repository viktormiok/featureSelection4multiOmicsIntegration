---
title: "Pathway Analysis for Cox regression Glmnet (lambda.min)"
author: "John Zhuang"
date: "2024-06-17"
...
The results here are deprecated in favor of lambda.1se Cox Glmnet.
...
```{r}
results.glmnet.cox = readRDS("data/sel_features/results_glmnet_cox_coef.rds")
...
Some microRNAs are dropped as they do not contribute enough to convert to genes.

```{r}
geneNames <- list("3F_0.5_DEV" = multiomics.to.GENES(names(sel.features$fold3$elasticnet_0.5$dev)),
                  "3F_0.5_C" = multiomics.to.GENES(names(sel.features$fold3$elasticnet_0.5$c)),
                  ...
                  "5F_0.5_DEV" = multiomics.to.GENES(names(sel.features$fold5$elasticnet_0.5$dev), drop.miRNA = T), 
                  "5F_0.5_C" = multiomics.to.GENES(names(sel.features$fold5$elasticnet_0.5$c)),
                  "5F_1_DEV" = multiomics.to.GENES(names(sel.features$fold5$lasso$dev), drop.miRNA = T),
                  "5F_1_C" = multiomics.to.GENES(names(sel.features$fold5$lasso$c)),
                  "10F_0.5_DEV" = multiomics.to.GENES(names(sel.features$fold10$elasticnet_0.5$dev)),
                  "10F_0.5_C" = multiomics.to.GENES(names(sel.features$fold10$elasticnet_0.5$c)),
                  "10F_1_DEV" = multiomics.to.GENES(names(sel.features$fold10$lasso$dev), drop.miRNA = T),
                  "10F_1_C" = multiomics.to.GENES(names(sel.features$fold10$lasso$c))
                  )
```
```{r}
geneNames
...
```{r}
saveRDS(geneNames, file = "data/sel_features/gene_converted/genes_glmnet_cox_min.rds")
```
...
saveRDS(pathway, file = "data/out_pathway/pathways_glmnet_cox.rds")
```
